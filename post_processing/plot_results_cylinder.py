    ...
    for c in range(n_cases):
        ...
        if mf_episodes and c > 0:
            if c == n_cases - 1:
                ...
                ax.scatter(mf_episodes[c], reward_mean[c][pt.tensor(mf_episodes[c])], color=color[c],
                           marker="x")
            else:
                ax.scatter(mf_episodes[c], reward_mean[c][pt.tensor(mf_episodes[c])], color=color[c],
                           marker="x")
